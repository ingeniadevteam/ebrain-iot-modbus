...
Modbus.prototype.read = async function (app) {
  ...
  const reqs = this.config.map(async conf => {
    ...
    if (conf.bus.type === 'TCP') {
      try {
        await client.connectTCP(conf.bus.host, { port: conf.bus.port });
        client.setTimeout(conf.bus.timeout);
        this.logger.debug(`modbus connected to ${conf.bus.host}:${conf.bus.port}`);
      } catch (error) {
...
Modbus.prototype.write = async function (outLabel, data) {
  while (!this.ready) await new Promise(res => setTimeout(() => res(), 50));
  this.ready = false;

  // the output
  let conf, output, parser, res;
  // get config
  ...
  try {
      switch (output.fc) {
        ...
        case 'writeRegister':
          if (typeof data === 'number') {
            await writeOutput(client, output, data /* * parser.scale*/);
          }
