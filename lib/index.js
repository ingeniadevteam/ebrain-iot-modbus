  ...
  const reqs = this.config.map(async conf => {
    ...
    if (conf.bus.type === 'TCP') {
      try {
        ...
        app.logger.debug(`modbus connected to ${device.bus.host}:${device.bus.port}`);
      } catch (error) {
  ...
  try {
      switch (output.fc) {
        case 'writeCoil':
          ...
          else this.app.logger.error(`modbus.writeCoil data must be a number`);
        break;
        case 'writeRegister':
          if (typeof data === 'number') {
            await writeOutput(client, output, data);
          }
          else this.app.logger.error(`modbus.writeRegister data must be a number`);
        break;
        ...
        case 'writeCoils':
          ...
          else this.app.logger.error(`modbus.writeCoils data must be an array`);
        break;
        case 'writeRegisters':
          if (Array.isArray(data)) {
            await writeOutput(client, output, data);
          }
          else this.app.logger.error(`modbus.writeRegisters data must be an array`);
        break;
        default:
            this.app.logger.error(`modbus.function ${output.fc} not supported`);
          break;
      }
  } catch (error) {
      this.app.logger.error(`modbus ${error.message}`);
  }
