...
Modbus.prototype.read = async function (app) {
  ...
  const reqs = this.config.map( async (conf) => {
    ...
    for (let i in conf.inputs) {
      // console.log('format', conf.inputs[i]);
      // read
      try {
        await new Promise( (resolve, reject) => {
          this.buses[conf.bus.name].client[conf.inputs[i].fc](
            conf.inputs[i].id, conf.inputs[i].add, conf.inputs[i].num,
            async (error, data) => {
            // reject error
            if (error) {
              // console.log(error.message, conf.inputs[i].id, conf.inputs[i].add, conf.inputs[i].num); // XXX
              return reject(error);
            }
            ...
            let unpacked;
            try {
              unpacked = bufferpack.unpack(conf.inputs[i].format, data.buffer, 0);
            } catch (e) {
              return reject(e);
            }
            ...
            if (unpacked) {
              ...
              console.log("ERROR: can not unpack", conf.inputs[i].id, conf.inputs[i].add, conf.inputs[i].num, data.buffer);
            }
            resolve();
          });
        });
  ...
  // return an error if there is no vars
  if (modbus_vars.length < 1) {
    this.logger.error(`modbus.read.length = ${modbus_vars.length}`);
  }
